...
@section('content')
    <div>
        <p class="text-center text-lg font-semibold">Edit data siswa</p>
    </div>
    <div class="flex justify-center mt-5">
        <div class="w-1/3 border-2 rounded-md py-10">
            <form method="POST" action="/update/{{ $siswa->Id_pendaftar }}" class="max-w-sm mx-auto">
                @csrf
                @method('PUT')
                <div class="mb-5">
                    <div>
                        ...
                        <input type="text" name="Nm_pendaftar" value="{{ $siswa->Nm_pendaftar }}" id="Nm_pendaftar"
                    ...
                    <div class="mt-5">
                        ...
                        <input type="text" name="No_hp" id="No_hp" value="{{ $siswa->No_hp }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @error('No_hp')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex justify-between mt-10">
                        <button type="submit" class="bg-green-600 w-32 h-10 rounded-md text-white"
                            onclick="return confirm('Ingin mengubah data siswa atas nama {{ $siswa->Nm_pendaftar }}')">Edit
                            data</button>
